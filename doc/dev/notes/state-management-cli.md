...
Arti will need the following new subcommands:
  ...
  * `arti-keys`, for general key management
...
NOTE: `arti-keys`, `arti-hsc` and `arti-hss` will be implemented as arti
subcommands (`arti keys`, `arti hsc`, `arti hss`).
...
DESCRIPTION
       ...
       most use cases, and low-level subcommands ("plumbing"), which offer
       finer-grained control but are less ergonomic and more dangerous to use.
...
PORCELAIN SUBCOMMANDS
       These are the commands most users will want to use.

       The subcommands describe high-level operations, manipulating individual
       keys or key bundles (as applicable). Users don't need to be familiar with
       the implementation details of Arti's on-disk key stores to use these
       commands.

       They also provide guardrails preventing users from corrupting the key
       stores, and are suitable for routine key management tasks.
...
PLUMBING SUBCOMMANDS
       The subcommands listed here operate on a lower level than the porcelain
       commands. You most likely do **not** need to use these. If you find
       yourself needing these commands for routine key management tasks, it
       means we are missing a porcelain wrapper for your workflow.

       Unlike the porcelain commands, these commands operate on individual keys,
       specified as `ArtiPath`s by the user. As such, their misuse can lead to
       key store corruption.

       list-raw           Plumbing subcommand for listing the files from key stores
       remove-by-raw-path Plumbing subcommand for removing files from key stores
...
### `arti-keys-list-raw` (plumbing)


```
NAME
       arti-keys-list-raw - List all available keys, without interpreting their
       meaning

SYNOPSIS
       arti keys list-raw [OPTIONS]
...
EXAMPLES
       List the keys in keystores foo and bar:
         ...
         arti keys list-raw --config arti.toml \
           --keystore foo,bar
...
### `arti-keys-remove-by-raw-path` (plumbing)

```
NAME
       arti-keys-remove-by-raw-path - Remove a key, given its ArtiPath

SYNOPSIS
       arti keys remove-by-raw-path [OPTIONS] <RAW_PATH>
...
EXAMPLES
       ...
       Remove a file from keystore foo:
         ...
         arti keys --config arti.toml remove-by-raw-path            \
             --keystore foo                                         \
             client/alice/abc.onion/ks_hsc_desc_enc.x25519_private
