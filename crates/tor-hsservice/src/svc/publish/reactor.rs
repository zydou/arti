...
impl TimePeriodContext {
    ...
    fn fetch_hsdirs<'r>(
        ...
        let hs_dirs = netdir.hs_dirs_upload(iter::once((blind_id, period)))?;
...
impl<R: Runtime, M: Mockable<R>> Reactor<R, M> {
    ...
    async fn upload_for_time_period(&self, current: bool) -> Result<(), ReactorError> {
        ...
        let hsdesc = if inner.descriptor.validate().is_ok() {
            let blind_id_kp = todo!();
            let mut rng = self.mockable.thread_rng();

            inner
                .descriptor
                .build_sign(self.hsid_key, blind_id_kp, context.period, &mut rng)?;
        } else {
            trace!(hsid=%self.hsid, "not enough information to build descriptor, skipping upload");
            return Ok(());
        };
