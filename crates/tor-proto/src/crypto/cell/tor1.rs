...
use tor_cell::{
    chancell::ChanCmd,
    relaycell::{msg::SendmeTag, RelayCellFields, RelayCellFormatTrait},
};
    ...
    for CryptStatePair<SC, D, RCF>
{
    ...
    fn initialize(mut seed: &[u8]) -> Result<Self> {
        ...
        let fwd = CryptState {
            ...
            last_digest_val: [0_u8; SENDME_TAG_LEN].into(),
...
impl RelayCellBody {
    ...
    fn set_digest<D: Digest + Clone, RCF: RelayCellFormatTrait>(
        &mut self,
        d: &mut D,
        used_digest: &mut SendmeTag, // XXXX rename.
    ) {
        ...
        *used_digest = SendmeTag::try_from(&computed_digest[..SENDME_TAG_LEN])
            .expect("Somehow produced a SENDME tag of invalid length!");
        let used_digest_prefix = &computed_digest[0..RCF::FIELDS::DIGEST_RANGE.len()];
