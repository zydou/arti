...
pub(crate) trait CryptInit: Sized {
    ...
    fn initialize(format: RelayCellFormat, seed: &[u8]) -> Result<Self>;
    /// Initialize this object from a key generator.
    fn construct<K: super::handshake::KeyGenerator>(
        format: RelayCellFormat,
        keygen: K,
    ) -> Result<Self> {
        let seed = keygen.expand(Self::seed_len())?;
        Self::initialize(format, &seed[..])
    }
}
...
pub(crate) mod tor1 {
    ...
    pub(crate) struct CryptStatePair<SC: StreamCipher, D: Digest + Clone> {
        ...
        /// Format of relay cells at this layer.
        format: RelayCellFormat,
    }
    ...
    impl<SC: StreamCipher + KeyIvInit, D: Digest + Clone> CryptInit for CryptStatePair<SC, D> {
        ...
        fn initialize(format: RelayCellFormat, mut seed: &[u8]) -> Result<Self> {
    ...
    impl<SC: StreamCipher, D: Digest + Clone> RelayCrypt for CryptStatePair<SC, D> {
        fn originate(&mut self, cell: &mut RelayCellBody) {
            ...
            cell.set_digest(self.format, &mut self.back.digest, &mut d_ignored);
        }
        fn encrypt_inbound(&mut self, cell: &mut RelayCellBody) {
        ...
        fn decrypt_outbound(&mut self, cell: &mut RelayCellBody) -> bool {
            ...
            let mut d_ignored = GenericArray::default();
            cell.is_recognized(self.format, &mut self.fwd.digest, &mut d_ignored)
        }
    }

    impl<SC: StreamCipher, D: Digest + Clone> OutboundClientLayer for CryptState<SC, D> {
        fn originate_for(&mut self, cell: &mut RelayCellBody) -> &[u8] {
            cell.set_digest(self.format, &mut self.digest, &mut self.last_digest_val);
    ...
    impl<SC: StreamCipher, D: Digest + Clone> InboundClientLayer for CryptState<SC, D> {
        fn decrypt_inbound(&mut self, cell: &mut RelayCellBody) -> Option<&[u8]> {
            ...
            self.cipher.apply_keystream(&mut cell.0[..]);
            if cell.is_recognized(self.format, &mut self.digest, &mut self.last_digest_val) {
...
mod test {
    ...
    fn roundtrip() {
        ...
        let pair =
            Tor1RelayCrypto::construct(RelayCellFormat::V0, KGen::new(seed1.clone())).unwrap();
        add_layers(&mut cc_out, &mut cc_in, pair);
        let pair =
            Tor1RelayCrypto::construct(RelayCellFormat::V0, KGen::new(seed2.clone())).unwrap();
        add_layers(&mut cc_out, &mut cc_in, pair);
        let pair =
            Tor1RelayCrypto::construct(RelayCellFormat::V0, KGen::new(seed3.clone())).unwrap();
        add_layers(&mut cc_out, &mut cc_in, pair);

        assert_eq!(cc_in.n_layers(), 3);
        assert_eq!(cc_out.n_layers(), 3);

        let mut r1 = Tor1RelayCrypto::construct(RelayCellFormat::V0, KGen::new(seed1)).unwrap();
        let mut r2 = Tor1RelayCrypto::construct(RelayCellFormat::V0, KGen::new(seed2)).unwrap();
        let mut r3 = Tor1RelayCrypto::construct(RelayCellFormat::V0, KGen::new(seed3)).unwrap();
    ...
    fn testvec() {
        ...
        let mut cc_out = OutboundClientCrypt::new();
        let mut cc_in = InboundClientCrypt::new();
        let pair = Tor1RelayCrypto::initialize(RelayCellFormat::V0, &K1[..]).unwrap();
        add_layers(&mut cc_out, &mut cc_in, pair);
        let pair = Tor1RelayCrypto::initialize(RelayCellFormat::V0, &K2[..]).unwrap();
        add_layers(&mut cc_out, &mut cc_in, pair);
        let pair = Tor1RelayCrypto::initialize(RelayCellFormat::V0, &K3[..]).unwrap();
        add_layers(&mut cc_out, &mut cc_in, pair);
