...
macro_rules! full_circuit_inbound_setup {
    ($sc:ty, $d:ty) => {{
        ...
        cc_in.add_layer_from_seed::<$sc, $d>(seed1).unwrap();
        cc_in.add_layer_from_seed::<$sc, $d>(seed2).unwrap();
        cc_in.add_layer_from_seed::<$sc, $d>(seed3).unwrap();

        let cell = create_inbound_cell::<$sc, $d>(&mut rng, &mut circuit_sates);
        (cell, cc_in)
    }};
}
...
fn create_inbound_cell<SC: StreamCipher + KeyIvInit, D: Digest + Clone>(
    rng: &mut impl rand::Rng,
    circuit_crypt_states: &mut [HopCryptState<SC, D>],
) -> RelayBody {
...
pub fn cell_decrypt_benchmark(c: &mut Criterion<CpuTime>) {
    ...
    group.bench_function("cell_decrypt_Tor1RelayCrypto", |b| {
        b.iter_batched_ref(
            || full_circuit_inbound_setup!(Aes128Ctr, Sha1),
            |(cell, cc_in)| {
    ...
    group.bench_function("cell_decrypt_Tor1Hsv3RelayCrypto", |b| {
        b.iter_batched_ref(
            || full_circuit_inbound_setup!(Aes256Ctr, Sha256),
            |(cell, cc_in)| {
                client_decrypt(cell, cc_in).unwrap();
            },
            criterion::BatchSize::SmallInput,
